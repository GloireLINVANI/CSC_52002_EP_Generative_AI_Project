...
def repaint_p_sample(
    ...
    if pred_xstart is not None:
        ...
        alpha_cumprod = extract_into_tensor(
            diffusion_model.alphas_cumprod, t, x.shape)
...
def rp_undo(diffusion_model, image_before_step, img_after_model, est_x_0, t, debug=False):
    beta = extract_into_tensor(diffusion_model.betas, t, img_after_model.shape)
    img_in_est = torch.sqrt(1 - beta) * img_after_model + torch.sqrt(beta) * torch.randn_like(img_after_model)
    return img_in_est

def repaint_p_sample_loop(diffusion_model, model, shape, device=None, noise=None,
                          clip_denoised=True, model_kwargs=None, progress=False,
                          jump_params=None, **kwargs):
...
def patch_model_for_repaint(diffusion):
    ...
    diffusion.p_sample = lambda *args, **kwargs: repaint_p_sample(diffusion, *args, **kwargs)
    diffusion.undo = lambda *args, **kwargs: rp_undo(diffusion, *args, **kwargs)
    diffusion.p_sample_loop = lambda *args, **kwargs: repaint_p_sample_loop(diffusion, *args, **kwargs)